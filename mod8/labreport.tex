...
But honestly, the import probably has some interesting contents. Lets construct a new url that tries to access this file (\verb|http://natas6.natas.labs.overthewire.org/includes/secret.inc|). This is the following contents of that page:
...
I am going to try and use the \verb|FOEIUWGHFEEUHOFUOIU| as the password on the main page and see if it gives me anything else. Yay! It worked! Here is the message that it displayed to me:
...
There seems to be a \verb|home| and \verb|about| page. I am going to visit both of them. The urls for them end up being \verb|http://natas7.natas.labs.overthewire.org/index.php?page=home| and \verb|http://natas7.natas.labs.overthewire.org/index.php?page=about| respectively.

When inspecting the page, I stumble across this comment in the html: \verb|<!-- hint: password for webuser natas8 is in /etc/natas_webpass/natas8 -->|. I am going to try and construct a new url, that might access this said file-path. Lets try \verb|http://natas7.natas.labs.overthewire.org/etc/natas_webpass/natas8|. This did not work. What if I try to use the url \verb|http://natas7.natas.labs.overthewire.org/index.php?page=/etc/natas_webpass/natas8|. This produced a page with the following contents (I assume the string is the password to the next one):
...
The output of my little helper function was the string "\verb|oubWYf2kBq|". After trying this as the password, the access was granted and I was given the page with this as the contents
