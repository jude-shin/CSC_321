...
This lab completes the OverTheWire CTF: Natas through level 16. A link to the game can be found \href{https://overthewire.org/wargames/natas/}{here}. As always, all the code can be found in our remote \href{https://github.com/jude-shin/CSC\_321}{GitHub repository}.
...
I used the same exact technique as level 0. \lstinline{ctrl+shift+i} to inspect the page, and then the password was found at the bottom of the source code. 
...
I see in the source code below that it puts the value I input, \lstinline{key}, directly into the
\lstinline{passthru} statement, which is executing the \lstinline{grep} command.
...
followed by \lstinline{#} to comment out the \lstinline{dictionary.txt} string.
...
Instead, I try \lstinline|ls /| to see the root directory. There is an \lstinline{etc} directory. 
...
The only difference between this level and the last one is a small blacklist:
...
The \lstinline{''} passes any empty match parameter to grep, so grep will print the entire file.  
I then insert the file I want to print, and use the same \lstinline{#} to ignore the rest of the line.
...
This generates: \lstinline{HmYkBwozJw4WNyAAFyB1VUc9MhxHaHUNAic4Awo2dVVHZ2FYU3NhWEc5}. I then set this as the cookie in my browser by using the following:
\begin{lstlisting}.
document.cookie="data=HmYkBwozJw4WNyAAFyB1VUc9MhxHaHUNAic4Awo2dVVHZ2FYU3NhWEc5"
\end{lstlisting}
Then, when I reload the page, it will use the cookie I just set, and it decodes that cookie to get the data array with showpassword set to "yes", which executes the line to show me the password for the next level.  
...
In this problem, they attempt to filter out characters that would allow command injection. However, blacklisting is not a secure way to filter input, and we can still command inject by submitting a string of the form: 
\begin{lstlisting}
$(grep ^<pw> /etc/natas_webpass/natas17)cringe
\end{lstlisting}
where pw is our guess at the password.

The key issue is that we can inject subcommands using \lstinline{\$()}, which is not blacklisted. Our command inside \lstinline{\$()} greps the natas17 file for a line starting with pw. The carrot forces the line to start with our password, so we aren't just matching any substring. If our guess matches the start of the password, the subcommand will output the password.

However, we don't have a way of viewing the password directly, instead, we append "cringe" to the end of the subcommand output, and look up the word in the dictionary. If our guess is correct, the full output will be \lstinline{"<password>cringe"}, which doesn't exist in the dictionary.txt file, so we will get a response with no words.
If our guess is incorrect, the output will be "cringe", which does exist in the dictionary.txt file, so we will get a response with the word "cringe". I use almost the exact same Cpp program as level 15, except with modification to the String being sent, and how the response is interpreted. For brevity, below I only provide the logic for sending the request, and interpreting the response.
